#Script for conducting PCA on any .vcf file written in a .txt file called variables.txt
...
PCA_plot <- ggplot(pcs, aes(PC1, PC2, color=as.factor(ploidy)))+
  geom_point(size=7)+
  labs(x=xlab, y=ylab, color="Ploidy")+
  geom_text_repel(aes(label=id), size=4, max.overlaps = 100,force=20, color="black")+
